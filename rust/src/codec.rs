...
impl NativeEncoder {
    ...
    pub fn encode_byte(&mut self, n: u8) {
        self.buffer.push(n);
    }
    ...
    pub fn encode_int32(&mut self, n: u32) {
    ...
    pub fn encode_bytes(&mut self, bytes: &Vec<u8>) {
        self.encode_int32(bytes.len() as u32);
...
impl NativeDecoder {
    ...
    pub fn decode_short(&mut self) -> Result<u16, &'static str> {
        match self.require(2) {
            true => {
                let ret = ((self.bytes[self.index] as u16) << 8)
                    | (self.bytes[self.index + 1] as u16);
...
mod test {
    ...
    pub fn test_codec() {
        ...
        encoder.encode_int32(u32::MIN);
        encoder.encode_int32(u32::MAX);
