...
impl JavaVM {
	...
	pub fn new_object_array(& self, class_name: &str, size: i32) -> Result<Object> {
		...
		let raw = unsafe {
			((**self.env).NewObjectArray)(self.env, size, clazz.raw, ptr::null_mut())
		};
	...
	pub fn new_byte_array(& self, size: i32) -> Result<Object> {
		let raw = unsafe {
			((**self.env).NewByteArray)(self.env, size)
		};
	...
	pub fn new_byte_array_with_data(& self, data: & Vec<u8>) -> Result<Object> {
		...
		unsafe {
			...
			((**self.env).SetByteArrayRegion)(self.env, array.raw, 0, data.len() as i32, src);
		}
	...
	pub fn load_byte_array(& self, array: &Object) -> Vec<u8> {
		let size = unsafe {
			((**self.env).GetArrayLength)(self.env, array.raw)
		};
		let mut result = vec![0u8; size as usize];
		unsafe {
			((**self.env).GetByteArrayRegion)(self.env, array.raw, 0, size, result.as_mut_ptr() as *mut i8);
		};
